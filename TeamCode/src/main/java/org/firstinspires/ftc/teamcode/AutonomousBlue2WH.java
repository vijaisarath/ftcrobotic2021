...
@Autonomous(name = "Autonomous Blue Warehouse", group = "Linear Opmode")
// @Disabled
public class AutonomousBlue2WH extends LinearOpMode {
    ...
    double motorMax = 0.92; // Limit motor power to this value for Andymark RUN_USING_ENCODER mode
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addData("DUck Left Position", "%.03f", duckLeftPosition);      //  ** ADDED **
        ...
        String position = "";
        if (duckLeftPosition == 0 || duckLeftPosition < 50) {
            telemetry.addData("Duck  Position", "Left");
            telemetry.update();
            position = "L";
            armPositionLow();
        } else if (duckLeftPosition <= 210) {
        ...
        moveBack(0.3, 0.90f);
        rotateAntiClockWise(0.2, 4.2f);

        moveBack(0.3, 11.45f);
        moveLeft(0.35, 12.5f);
        moveFront(0.25, 5.6f);

        rotateAntiClockWise(0.2, 4.8f);
    ...
    private float detectObject() {
        ...
        int round = 1;
        boolean isDuckOrCubeDetected = false;     //  ** ADDED **
        if (opModeIsActive()) {
...
//                            telemetry.addData(String.format("  right,bottom (%d)", i), "%.03f , %.03f", recognition.getRight(), recognition.getBottom());
                            ...
                            if (recognition.getLabel().equals("Duck") || recognition.getLabel().equals("Cube")) {            //  ** ADDED **
                                isDuckOrCubeDetected = true;                             //  ** ADDED **
                                duckLeftPosition = recognition.getLeft();
                                objectDetected = recognition.getLabel();
                                //telemetry.addData("Object Detected", objectDetected);      //  ** ADDED **
                            } else {                                               //  ** ADDED **
                                duckLeftPosition = recognition.getLeft();
                                objectDetected = recognition.getLabel();
                                //telemetry.addData("Object Detected", objectDetected);      //  ** ADDED **
                                isDuckOrCubeDetected = false;                            //  ** ADDED **
                            }                                                      //  ** ADDED **
                        }
        ...
        telemetry.addData("What Detected", objectDetected);      //  ** ADDED **
        telemetry.addData("Round", round);      //  ** ADDED **
        telemetry.addData("Object Detected", isDuckOrCubeDetected);      //  ** ADDED **
